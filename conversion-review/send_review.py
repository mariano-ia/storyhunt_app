...
Usage:
    python3 send_review.py analysis.json [metrics.json]
"""
...
def render_html(analysis, metrics_window):
    ...
    def proposal_card(p):
        ...
        <div style="border:1px solid #e2e8f0;border-radius:8px;padding:16px 18px;margin-bottom:14px;background:#fafafa;">
          <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:8px;">
            <span style="font-family:'SF Mono',Menlo,monospace;font-size:11px;color:#94a3b8;letter-spacing:.08em;">PROPUESTA #{esc(rank)}</span>
...
<div style="max-width:640px;margin:0 auto;padding:32px 20px;">
  ...
  <div style="background:#fff;padding:24px;border-left:1px solid #e2e8f0;border-right:1px solid #e2e8f0;">
    ...
    <div style="font-size:11px;font-family:'SF Mono',Menlo,monospace;color:#94a3b8;letter-spacing:.1em;margin-bottom:8px;">MÉTRICAS</div>
...
def main():
    if len(sys.argv) < 2:
        sys.exit("Usage: send_review.py analysis.json [metrics.json]")
    ...
    subject = f"StoryHunt /start{period} · {km.get('start_pageviews','?')} visitas · {km.get('purchase','?')} compras"

    html = render_html(analysis, metrics_window)
